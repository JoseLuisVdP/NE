...
func scan() -> Array[Resource]:
	...
	var blacklistFile = FileAccess.open("res://addons/godot_resource_groups/folders_blacklist.json", FileAccess.READ)
	blacklist = JSON.parse_string(blacklistFile.get_as_text())["blacklist"]
	var disabled = JSON.parse_string(blacklistFile.get_as_text())["disabled"] != "1"
