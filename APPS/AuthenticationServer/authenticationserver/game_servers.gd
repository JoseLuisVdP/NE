...
func save_game_data(data:Dictionary, player_id:int, server_id:int = 1) -> void:
	get_node_or_null("/root/Authenticate/SaveAndLoad").save_game(data, player_id, server_id)
...
func get_player_savefile(savefile:String, player_id:int, server_id:int) -> void:
	...
	get_node_or_null("/root/Authenticate/SaveAndLoad").load_game(savefile, player_id, server_id)
